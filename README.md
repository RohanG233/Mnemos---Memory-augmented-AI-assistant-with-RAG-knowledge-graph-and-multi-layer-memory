# ACAI — AI-Augmented Conversational Intelligence
...
ACAI (AI-Augmented Conversational Intelligence) is a personal AI assistant that remembers you.

Unlike a stateless chat interface, ACAI maintains:
...
│   │   │   ├── useDocuments.ts  # (empty — logic lives in useUpload)
...
| `MONGODB_DATABASE` | No | `acai` | Database name |
...
git remote add origin https://github.com/<your-username>/acai.git
...
3. Connect your GitHub account → select your `acai` repo
4. Render will detect `render.yaml` and show two services:
   - `acai-backend` (Web Service)
   - `acai-frontend` (Static Site)
...
In the Render dashboard → `acai-backend` → Environment:
...
| `GOOGLE_REDIRECT_URI` | `https://acai-backend.onrender.com/auth/google/callback` |
| `FRONTEND_URL` | `https://acai-frontend.onrender.com` |
| `ALLOWED_ORIGINS` | `https://acai-frontend.onrender.com` |
...
In the Render dashboard → `acai-frontend` → Environment:

| Key | Value |
|---|---|
| `VITE_API_URL` | `https://acai-backend.onrender.com` |
...
2. In Render dashboard → `acai-backend` → Settings → Start Command:
```
bash start.sh
```
...
2. Authorized redirect URIs → Add URI:
   ```
   https://acai-backend.onrender.com/auth/google/callback
   ```
3. Authorized JavaScript origins → Add:
   ```
   https://acai-frontend.onrender.com
   ```
4. Save
...
1. Go to `acai-backend` → Manual Deploy → Deploy latest commit
2. Watch the logs — startup should show:
   ```
   MongoDB connection OK (database=acai)
   ChromaDB collections ready.
   ACAI backend started. Serving requests.
   ```
3. Go to `acai-frontend` → Manual Deploy → Deploy latest commit
...
```
GET https://acai-backend.onrender.com/health
→ {"status": "healthy"}

GET https://acai-backend.onrender.com/health/dependencies
→ {"mongodb": "ok", "ollama": "ok"}  # ollama only ok if configured

GET https://acai-frontend.onrender.com
→ ACAI login page
```
...
- The `JWT_SECRET_KEY` exposed in previous sessions should be considered compromised and rotated
- The Google OAuth client secret exposed in previous sessions should be regenerated
- The MongoDB password exposed in previous sessions should be changed
